...
def generate_pdf_report(patient, assessment, result):
    ...
    header_lines = ["MedFusion"]
    if doctor and doctor.clinic_name:
        header_lines.append(doctor.clinic_name)

    elements.append(Paragraph(header_lines[0], title_style))
    if len(header_lines) > 1:
        elements.append(Paragraph(header_lines[1], styles["Heading4"]))
    ...
    patient_table_data = [
        ...
        ["Attending Doctor", f"Dr. {patient.doctor.name}" if patient.doctor else "N/A"],
    ]
    ...
    elements.append(Paragraph(
        "Disclaimer: This report is generated by an clinical decision-support "
        "prototype (MedFusion) developed for academic purposes. It is not a substitute "
    ...
    if doctor:
        signature_style = ParagraphStyle(
            "Signature", parent=styles["Normal"], fontName="Helvetica-Oblique", fontSize=11
        )
        elements.append(Paragraph("_" * 30, normal))
        elements.append(Paragraph(f"Dr. {doctor.name}", signature_style))
        if doctor.specialization:
            elements.append(Paragraph(doctor.specialization, normal))
